...
import { useRealtimeAllBesoins } from './useRealtimeAllBesoins'
...
describe('useRealtimeAllBesoins', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    const mockSubscribe = vi.fn().mockReturnValue({ id: 'test-channel' })
    const mockOn = vi.fn().mockReturnValue({ subscribe: mockSubscribe })
  ...
  it('subscribes to besoins:global channel without filter', () => {
    renderHook(() => useRealtimeAllBesoins(), { wrapper: createWrapper() })

    expect(supabase.channel).toHaveBeenCalledWith('besoins:global')
  })

  it('listens to postgres_changes on besoins table without filter', () => {
    renderHook(() => useRealtimeAllBesoins(), { wrapper: createWrapper() })
  ...
  it('removes channel on unmount', () => {
    const { unmount } = renderHook(() => useRealtimeAllBesoins(), {
  ...
  it('invalidates all-pending-besoins-count query when realtime callback fires', () => {
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    })
    ...
    renderHook(() => useRealtimeAllBesoins(), { wrapper: Wrapper })
