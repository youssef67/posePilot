...
import { useRealtimeLots } from './useRealtimeLots'
...
describe('useRealtimeLots', () => {
  ...
  it('subscribes to lots-changes-{plotId} channel on mount', () => {
    renderHook(() => useRealtimeLots('plot-1'), { wrapper: createWrapper() })

    expect(supabase.channel).toHaveBeenCalledWith('lots-changes-plot-1')
  })

  it('listens to postgres_changes on lots table', () => {
    renderHook(() => useRealtimeLots('plot-1'), { wrapper: createWrapper() })
  ...
  it('removes channel on unmount', () => {
    const { unmount } = renderHook(() => useRealtimeLots('plot-1'), {
  ...
  it('invalidates lots query when realtime callback fires', () => {
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    })
    ...
    renderHook(() => useRealtimeLots('plot-1'), { wrapper: Wrapper })
