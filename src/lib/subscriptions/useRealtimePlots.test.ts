...
import { useRealtimePlots } from './useRealtimePlots'
...
describe('useRealtimePlots', () => {
  ...
  it('subscribes to plots-changes-{chantierId} channel on mount', () => {
    renderHook(() => useRealtimePlots('chantier-1'), { wrapper: createWrapper() })

    expect(supabase.channel).toHaveBeenCalledWith('plots-changes-chantier-1')
  })

  it('listens to postgres_changes on plots table', () => {
    renderHook(() => useRealtimePlots('chantier-1'), { wrapper: createWrapper() })
  ...
  it('removes channel on unmount', () => {
    const { unmount } = renderHook(() => useRealtimePlots('chantier-1'), {
  ...
  it('invalidates plots query when realtime callback fires', () => {
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    })
    ...
    renderHook(() => useRealtimePlots('chantier-1'), { wrapper: Wrapper })
